...
4. **Eligibility-rule freezing** — a deterministic cross-dataset candidate now exists, but remains `CANDIDATE_ONLY`. It uses Company Census `status_code` and `carrier_operation` plus MOTUS `op_auth_type` and `op_auth_status`; unresolved `docket1_status_code` and `review_type` are explicitly prohibited dependencies. Freeze remains blocked on digest-bound preservation of the exact MOTUS value semantics, a point-in-time MOTUS input joined by USDOT number, and temporal preregistration.
