...
4. **Eligibility-rule freezing** — a deterministic cross-dataset candidate now exists, but remains `CANDIDATE_ONLY`. It uses Company Census `status_code` and `carrier_operation` plus MOTUS `op_auth_type` and `op_auth_status`; unresolved `docket1_status_code` and `review_type` are explicitly prohibited dependencies. The exact MOTUS value semantics are preserved and digest-bound. Freeze remains blocked on a point-in-time MOTUS input joined by USDOT number, temporal alignment/preregistration, and the existing complete-frame evidence blockers.
...
Current determination: `BOUNDED_COHORT_AUDIT_PASSED`; `SEMANTIC_BINDING_PARTIALLY_BOUND`; `ELIGIBILITY_RULE_NOT_FROZEN`. These are pipeline states, not evidence that any carrier is eligible or that any platform gate is valid or invalid.
