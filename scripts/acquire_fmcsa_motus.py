#!/usr/bin/env python3
"""Acquire a version-bound, deterministically ordered FMCSA MOTUS authority frame."""
...
DATASET_NAME = "Motus Carrier - All With History"
...
ORDER = ":id ASC"
SELECT = ":id,docket_number,usdot_number,rfc_number,op_auth_type,op_auth_status,min_cov_amount,cargo_req,bond_req,bipd_file,cargo_file,bond_file,bus_undeliverable_mail,mail_undeliverable_mail,dba_name,legal_name,bus_street_po,bus_colonia,bus_city,bus_state_code,bus_ctry_code,bus_zip_code,bus_telno,mail_street_po,mail_colonia,mail_city,mail_state_code,mail_ctry_code,mail_zip_code"
USER_AGENT = "trucking-software-research/3.0"
CHECKPOINT_FORMAT = 1
DEFAULT_OUT = Path("data/raw/fmcsa/motus-carrier-all-history.json")
DEFAULT_MANIFEST = Path("data/derived/fmcsa/motus-acquisition-manifest.json")
DEFAULT_SCHEMA = Path("data/fmcsa/motus-schema.json")
METADATA_ARTIFACT = Path("data/fmcsa/authoritative/motus-carrier-all-history-metadata-2026-08-30.json")
METADATA_SHA256 = "5305300ed434eaa2271ff8788cd2692b1094460ed22cd3bdbc80f693dccaba2e"
REQUIRED_FIELDS = {"usdot_number", "op_auth_type", "op_auth_status"}
...
def metadata_identity(path=METADATA_ARTIFACT):
    path = Path(path)
    try:
        exact_digest = hashlib.sha256(path.read_bytes()).hexdigest()
        metadata = json.loads(path.read_bytes())
    except (OSError, json.JSONDecodeError) as error:
        raise CompleteFrameBlocked(f"preserved MOTUS metadata is unavailable or malformed: {error}") from error
    if exact_digest != METADATA_SHA256:
        raise CompleteFrameBlocked("preserved MOTUS metadata does not match its required exact-byte digest")
    if metadata.get("id") != DATASET_ID or metadata.get("rowsUpdatedAt") != 1787999953:
        raise CompleteFrameBlocked("preserved MOTUS metadata identity/version is not the bound T0 observation")
    return {"artifact_path": str(path), "exact_byte_digest": "sha256:" + exact_digest,
            "observed_at": "2026-08-30T04:41:28Z", "rows_updated_at": "1787999953"}
...
def schema_binding(columns, retrieved_at):
    ...
    available = {field["field"] for field in fields}
    missing = sorted(REQUIRED_FIELDS - available)
    if missing:
        raise CompleteFrameBlocked(f"official schema lacks required candidate fields: {missing}")
    return {
...
def page_url(limit, offset):
    return BASE + "?" + urlencode({
        "$select": SELECT, "$limit": limit, "$offset": offset, "$order": ORDER,
    })
...
def validate_checkpoint_state(state, page_size):
    ...
    if (total == 0 and previous is not None) or (total > 0 and not isinstance(previous, str)):
        raise CompleteFrameBlocked("checkpoint ordering state is inconsistent")
    seen_ids = state.get("seen_ids")
    if (not isinstance(seen_ids, list) or any(not isinstance(value, str) for value in seen_ids)
            or len(seen_ids) != total or len(set(seen_ids)) != total
            or (seen_ids and seen_ids[-1] != previous)):
        raise CompleteFrameBlocked("checkpoint :id uniqueness state is inconsistent")
...
def validate_id(row):
    value = row.get(":id")
    if value is None or value == "" or isinstance(value, bool) or not isinstance(value, (str, int)):
        raise CompleteFrameBlocked("missing or invalid :id prevents a unique ordered MOTUS frame")
    return str(value)


def validate_usdot_number(row):
    value = row.get("usdot_number")
    if value is None or value == "" or isinstance(value, bool) or not isinstance(value, (str, int)):
        raise CompleteFrameBlocked("missing or invalid usdot_number prevents an eligibility-joinable MOTUS frame")
    try:
        parsed = int(value)
        if parsed <= 0:
            raise ValueError
        ...
        raise CompleteFrameBlocked("missing or invalid usdot_number prevents an eligibility-joinable MOTUS frame") from error


def acquire(transport, out, manifest_path, schema_path, page_size=50000, retries=3,
            clock=utc_now, sleep=time.sleep, checkpoint_path=None, metadata_path=METADATA_ARTIFACT):
    if page_size < 1:
    ...
    preserved_metadata = metadata_identity(metadata_path)
    ...
    observed_binding["preserved_metadata_identity"] = preserved_metadata
    ...
    try:
        while not state["terminal_page_received"]:
            ...
            candidate_previous = state["previous_id"]
            candidate_seen = set(state["seen_ids"])
            for row in page:
                id_value = validate_id(row)
                validate_usdot_number(row)
                if id_value in candidate_seen:
                    raise CompleteFrameBlocked("duplicate :id violates the source identity contract")
                candidate_seen.add(id_value)
                candidate_previous = id_value
            ...
            state = {
                ...
                "previous_id": candidate_previous,
                "seen_ids": state["seen_ids"] + [validate_id(row) for row in page],
        ...
        manifest = {
            ...
            "preserved_metadata_identity": preserved_metadata,
            "acquisition_started_at": started,
            "acquisition_completed_at": completed,
            "query_contract": query_contract(page_size),
            "ordering_contract": contract["ordering_contract"],
            "identifier_semantics": ":id is used only as Socrata acquisition order/system identity; no trucking-domain meaning is inferred.",
            "duplicate_semantics": "Duplicate usdot_number and docket_number values are legitimate operating-authority records and are preserved without deduplication.",
            "retry_contract": {
                "retries": retries,
            ...
            "known_limitations": [
                ...
                "The acquired frame is not temporally aligned to a Company Census frame and does not freeze or execute the candidate eligibility rule.",
            ],
        }
